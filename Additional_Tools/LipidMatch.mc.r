...
if (ManuallyInputVariables==TRUE){
  ...
  InputLibrary<-"/home/jmun0029/Documents/data2/GitHub/LipidMatch/LipidMatch_Libraries/"
  ...
  #parametersDirectory
  parametersDir <- "/home/jmun0029/GarrettLab-UF/LipidMatch/"
  #parametersDir <- "/home/jmun0029/Documents/data2/GitHub/testdata/"
  parametersFile <- paste(parametersDir, "LIPIDMATCH_PARAMETERS_Thermo_QExactive_Ubuntu.csv", sep="")
...
for(i in seq_len(lengthFoldersToRun)){
  ...
  if(runNegddMS && nrow(LibraryCriteria)>0){
    ...
    for (c in 1:length(ddMS2NEG_in)){
      ...
      foreach (i = seq_len(nrow(LibraryCriteria)), .options.multicore=mcoptions, .inorder = FALSE) %dopar% {
